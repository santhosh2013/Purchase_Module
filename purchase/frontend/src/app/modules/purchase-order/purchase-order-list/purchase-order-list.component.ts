...
export class PurchaseOrderListComponent implements OnInit {
  ...
  applyFilters(): void {
    this.filteredOrders = this.purchaseOrders.filter(order => {
      ...
      const matchesEventId = !this.filterEventId || order.eventid === Number(this.filterEventId);
      const matchesVendorName = !this.filterVendorName || 
        order.vendorname.toLowerCase().includes(this.filterVendorName.toLowerCase());
      return matchesStatus && matchesEventId && matchesVendorName;
    });
  ...
  deletePurchaseOrder(id: number): void {
    this.toastr.warning('Deleting purchase order...', 'Confirm', {
      timeOut: 5000,
      closeButton: true,
      tapToDismiss: false
    }).onTap.subscribe(() => {
